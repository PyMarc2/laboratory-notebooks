...
def analyze_edge(file, imagevoxelsize=0.25, linelength=None):
    ...
    # Eliminates the artefact that is present when the profile is drawn outside of the phantom :
    if erf[0] > 0.5:
        erf = erf[np.where(erf < 0.5)[0][0]:-1]
        pos = pos[np.where(erf < 0.5)[0]:-1]
    
    # Arbitraty clipping of the profile at the beginning and at the end so the ERF is centered on the x axis :
    ...
    # Creating a spline on the ERF fit in order to get a numerical derivative on the same x axis as the ERF
    ...
    plt.xlabel("Position ($x$) [mm]", fontsize=16)
    ...
    plt.plot(freq, mtf)
    plt.xlim(0, 1.2)
    plt.xlabel("Fréquence spatiale ($f$) [mm$^{-1}$]", fontsize=16)
    plt.ylabel("MTF($f$)", fontsize=16)
    plt.tick_params(labelsize=16)
    plt.tight_layout()
